...
class DpkgDeb:
    def download_linux_64(args):
        ...
        res = requests.get(
            "http://ftp.us.debian.org/debian/pool/main/d/dpkg/dpkg_1.21.9_amd64.deb",
            stream=True)
        try:
    ...
    def download_linux_arm64(args):
        ...
        res = requests.get(
            "http://ftp.us.debian.org/debian/pool/main/d/dpkg/dpkg_1.21.9_arm64.deb",
            stream=True)
        try:
            if res.status_code == 200:
                with open(f"dpkg.deb", "wb") as f:
